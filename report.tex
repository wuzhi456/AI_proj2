...
\title{Adult Census Income Prediction Using Simplified Neural Networks}
...
\begin{abstract}
This report presents a simplified neural network-based approach to predict whether an individual's income exceeds \$50,000 per year based on census data from the 1994 US Census Bureau database. We implement a streamlined multi-layer perceptron (MLP) using only ReLU activations with a softmax output layer, focusing on a clean architecture without batch normalization or dropout. The proposed model achieves competitive performance on the validation set with proper handling of categorical features and missing values. Experimental results demonstrate the effectiveness of straightforward deep learning architectures for this binary classification task.
\end{abstract}
...
\begin{itemize}
    ...
    \item $\text{softmax}(\cdot)$ denotes the softmax activation function
...
We employ a simplified feed-forward multi-layer perceptron (MLP) with the following architecture:

\begin{algorithm}[h]
\caption{Forward Pass of Neural Network}
\begin{algorithmic}
\STATE \textbf{Input:} Feature vector $\mathbf{x} \in \mathbb{R}^d$
\STATE \textbf{Output:} Probability vector $\mathbf{p} \in [0,1]^2$
...
\begin{itemize}
    ...
    \item \textbf{Output Layer}: Two neurons with softmax activation for binary classification
\end{itemize}
...
\end{equation}
where $C=2$ is the number of classes, $y_{i,c}$ is the true label (one-hot encoded), and $\hat{y}_{i,c}$ is the predicted probability for class $c$.
...
\begin{itemize}
    \item \textbf{Framework}: PyTorch 2.9.1
    \item \textbf{Hardware}: CPU (Intel/AMD x86\_64 architecture)
    \item \textbf{Python Version}: 3.12
    \item \textbf{Additional Libraries}: NumPy 2.3.5, Pandas 2.3.3, Scikit-learn 1.7.2, Matplotlib 3.10.7
\end{itemize}
...
\begin{itemize}
    \item \textbf{Network Depth}: Tested architectures with 2-4 hidden layers. Three layers provided the best balance between capacity and simplicity.
    ...
    \item \textbf{Architecture Simplicity}: A clean architecture with only ReLU activations proved effective, eliminating the need for batch normalization or dropout.
\end{itemize}

\subsection{Analysis and Discussion}

\subsubsection{Model Performance}
The simplified neural network achieves competitive performance on this dataset. The high ROC-AUC score indicates strong discriminative ability between income classes. The model benefits from:
\begin{itemize}
    \item Clean architecture with straightforward ReLU activations
    \item Softmax output providing well-calibrated probability estimates
...
\subsection{Summary}
This project successfully applied neural networks to the Adult Census Income prediction problem. Our simplified multi-layer perceptron with careful preprocessing achieves strong performance on the validation set. The methodology demonstrates the effectiveness of clean, straightforward deep learning architectures for tabular data classification tasks.

\subsection{Key Findings}
\begin{itemize}
    \item Proper data preprocessing is crucial for neural network performance
    \item A simplified architecture with only ReLU activations is effective
    \item Softmax activation provides well-calibrated probability outputs
...
\begin{itemize}
    ...
    \item The simplified architecture may underfit compared to more complex models with regularization
\end{itemize}
...
\begin{itemize}
    ...
    \item Testing regularization techniques (batch normalization, dropout, L2)
